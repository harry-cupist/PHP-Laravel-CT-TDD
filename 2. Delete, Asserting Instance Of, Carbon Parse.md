...
동일한 방법으로 데이터 생성 후, 상세 페이지(show)로 리다이렉트 설정
...
그러나, 업데이트 부분에서 에러가 발생함. 이는 여전히 변경 전 title을 url에 반영하고 있기 때문임. 따라서 새롭게 업데이트된 title로 redirect 시키도록, 인스턴스를 새로고침하는 `fresh()` 메소드를 추가.
...
포맷팅을 맞추기 위해서는 모델 클래스에서 메소드를 구현하되, 컨벤션 룰을 정확히따라줘야함. 인수에는 $attribute를 넣어도 되며, 우리는 이 상황에서 $dob가 들어오는 것을 알고있으므로 간단히 $dob를 인수로 받아옴.
